...
exports.default = {
    ...
    run: async (context) => {
        ...
        if (context.manager) {
            ...
            const totalUsage = results.reduce((x, info) => x + info.usage, 0);
            rows.push([
                'T',
                String(results.reduce((x, info) => x + info.guilds, 0)),
                String(results.reduce((x, info) => x + info.members, 0)),
                `${Math.round(totalUsage / 1024 / 1024)}`,
            ]);
        }
        const paddedRows = utils_1.padCodeBlockFromRows(rows, { join: '| ' });
        const largestRow = paddedRows.reduce((x, row) => Math.max(x, row.length), 0);
        paddedRows.splice(1, 0, '-'.repeat(largestRow));
        return context.editOrReply([
            '```py',
            utils_1.padCodeBlockFromRows(title).join('\n') + '\n',
            paddedRows.join('\n'),
            '```',
        ].join('\n'));
    },
