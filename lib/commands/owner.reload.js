...
exports.default = {
    ...
    run: async (context) => {
        ...
        const shardIds = await context.manager.broadcastEval(async (cluster) => {
            ...
            return cluster.shards.map((shard) => shard.shardId);
        });
        const error = shardIds.find((shardId) => shardId instanceof Error);
        if (error) {
